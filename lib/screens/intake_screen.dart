...
class _ViewMock {
  ...
  const _ViewMock({
    ...
    required this.conf,  required this.urgent,
  });
}
...
class _DemoScenario {
  ...
  const _DemoScenario({
    required this.views, required this.status,
    required this.finding, required this.confidence,
    required this.impression, required this.resultTitle,
    required this.resultSub, required this.bannerText,
...
const _scenarios = [
  ...
  _DemoScenario(
    views: {
      'L-CC':  _ViewMock(label: 'Spiculated mass',       detail: '14 mm · upper-outer quad.', conf: 0.88, urgent: true),
      'L-MLO': _ViewMock(label: 'Corroborating density', detail: 'Overlapping tissue',         conf: 0.61, urgent: false),
      'R-CC':  _ViewMock(label: 'No finding',            detail: 'Normal breast tissue',       conf: 0.95, urgent: false),
      'R-MLO': _ViewMock(label: 'No finding',            detail: 'Normal breast tissue',       conf: 0.93, urgent: false),
    },
    ...
    resultTitle: 'Urgent — review required',
    resultSub: 'Case assigned to urgent queue',
    bannerText: 'AI analysis complete · Spiculated mass detected in L-CC — case flagged urgent.',
    bannerBadge: '88% conf.',
  ),
  // 1 — Auto-cleared
  _DemoScenario(
    views: {
      'L-CC':  _ViewMock(label: 'No finding', detail: 'Normal breast tissue', conf: 0.96, urgent: false),
      'L-MLO': _ViewMock(label: 'No finding', detail: 'Normal breast tissue', conf: 0.94, urgent: false),
      'R-CC':  _ViewMock(label: 'No finding', detail: 'Normal breast tissue', conf: 0.97, urgent: false),
      'R-MLO': _ViewMock(label: 'No finding', detail: 'Normal breast tissue', conf: 0.95, urgent: false),
    },
    ...
    resultTitle: 'Auto-cleared ✓',
    resultSub: 'High-confidence bilateral screening',
    bannerText: 'AI analysis complete · No significant findings — bilateral screening cleared.',
    bannerBadge: '95% conf.',
  ),
  // 2 — Needs review
  _DemoScenario(
    views: {
      'L-CC':  _ViewMock(label: 'No finding',        detail: 'Normal breast tissue',        conf: 0.89, urgent: false),
      'L-MLO': _ViewMock(label: 'No finding',        detail: 'Normal breast tissue',        conf: 0.91, urgent: false),
      'R-CC':  _ViewMock(label: 'Focal asymmetry',   detail: 'Requires spot compression',   conf: 0.63, urgent: false),
      'R-MLO': _ViewMock(label: 'Asymmetric density', detail: 'Cannot exclude distortion',  conf: 0.58, urgent: false),
    },
    ...
    resultTitle: 'Needs radiologist review',
    resultSub: 'Case added to review queue',
    bannerText: 'AI analysis complete · Focal asymmetry in R-CC — spot compression recommended.',
    bannerBadge: '63% conf.',
  ),
];
...
class _IntakeScreenState extends State<IntakeScreen> {
  ...
  @override
  void dispose() { _ageCtrl.dispose(); super.dispose(); }

  int  get _uploaded => _scanStates.values.where((s) => s == _ScanState.done).length;
  ...
  void _runTriage() {
    ...
    final scenarioIdx  = _scenarioIndex % _scenarios.length;
    setState(() => _triageState = _TriageState.running);
    Future.delayed(const Duration(milliseconds: 2400), () {
      if (!mounted) return;
      _intakeCounter++;
      _scenarioIndex++;
      setState(() => _triageState = _TriageState.complete);
  ...
  _StepState _step(int i) {
    switch (i) {
      ...
      case 2:
        ...
        if (_triageState == _TriageState.running || _triageState == _TriageState.ready)
          return _StepState.active;
        return _StepState.future;
      case 3:
        return _triageState == _TriageState.complete ? _StepState.active : _StepState.future;
      default: return _StepState.future;
    }
  }
      ...
      child: Column(children: [
        ...
        TCard(
          ...
          child: Row(children: [
            ...
            Expanded(child: _EditableField(label: 'Age / Sex', controller: _ageCtrl)),
            const SizedBox(width: 12),
            Expanded(child: _DropdownField(
              label: 'Study type',
              value: _studyType,
              items: const [
                'Bilateral screening', 'Diagnostic', 'Unilateral screening',
            ...
            Expanded(child: _DropdownField(
              ...
              items: const [
                'None', 'Yes — 1st degree', 'Yes — 2nd degree',
                'Yes — multiple relatives', 'Unknown',
              ],
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(flex: 7, child: TCard(padding: const EdgeInsets.all(18), child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Row(children: [
                Text('MAMMOGRAM VIEWS', style: GoogleFonts.dmSans(
                ...
                AnimatedSwitcher(
                  ...
                  child: Text('$_uploaded / 4 uploaded',
                    ...
                    style: GoogleFonts.dmSans(fontSize: 11,
                      color: _allDone ? TulipColors.green400 : TulipColors.textS)),
                ),
              ]),
          ...
          Expanded(flex: 5, child: Column(children: [
            TCard(padding: const EdgeInsets.all(18), child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Routing logic', style: GoogleFonts.dmSans(
                    fontSize: 13, fontWeight: FontWeight.w600, color: TulipColors.text)),
                const SizedBox(height: 12),
                _RoutingCard(title: 'Confidence > 85% — Auto-clear',
                  body: 'Pre-read report sent to radiologist for sign-off.',
                  color: TulipColors.green50, textColor: TulipColors.green800),
                const SizedBox(height: 8),
                _RoutingCard(title: 'Confidence 60–85% — Needs review',
                  body: 'Routed to worklist, sorted by urgency score.',
                  color: TulipColors.amber50, textColor: TulipColors.amber800),
                const SizedBox(height: 8),
                _RoutingCard(title: 'Confidence < 60% — High priority',
                  body: 'Pushed to urgent queue with immediate notification.',
                  color: TulipColors.red50, textColor: TulipColors.red800),
              ],
            )),
            const SizedBox(height: 16),
            _StatusCard(
...
class _ScanTile extends StatelessWidget {
  ...
  const _ScanTile({
    required this.view, required this.state, required this.mock,
    required this.showResult, required this.onTap, this.imageBytes,
  });
          ...
          child: ClipRRect(
            ...
            child: Stack(fit: StackFit.expand, children: [
              ...
              if (state == _ScanState.empty)
                ...
                _DoneContent(
                  view: view, mock: mock,
                  showResult: showResult, accent: _accent,
                ),
            ]),
          ),
        ),
  ...
  Widget build(BuildContext context) => Center(child: Column(
    ...
    children: [
      Container(
        ...
        child: const Icon(Icons.add_photo_alternate_outlined, size: 20, color: Colors.white38),
      ),
      const SizedBox(height: 8),
      Text(view, style: GoogleFonts.dmSans(
          fontSize: 13, fontWeight: FontWeight.w700, color: Colors.white70)),
      const SizedBox(height: 3),
      Text('Click to upload', style: GoogleFonts.dmSans(
...
class _LoadingContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(
    ...
    children: [
      const SizedBox(
        width: 28, height: 28,
        child: CircularProgressIndicator(strokeWidth: 2, color: TulipColors.p400),
      ),
      const SizedBox(height: 10),
      Text('Processing…', style: GoogleFonts.dmSans(fontSize: 10, color: Colors.white38)),
    ],
  ));
}
...
class _DoneContent extends StatelessWidget {
  ...
  final Color accent;
  const _DoneContent({
    required this.view, required this.mock,
    required this.showResult, required this.accent,
  });
  @override
  Widget build(BuildContext context) => Stack(fit: StackFit.expand, children: [
    ...
    // View label
    Positioned(
      bottom: showResult ? 44 : 7, left: 7,
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
        decoration: BoxDecoration(
          color: Colors.black54, borderRadius: BorderRadius.circular(4)),
        child: Text(view, style: GoogleFonts.dmSans(fontSize: 10, color: Colors.white60)),
      ),
    ),
    // Result overlay
    if (showResult) Positioned(
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(mock.label, style: GoogleFonts.dmSans(
              fontSize: 10, fontWeight: FontWeight.w600, color: accent)),
          Text('${(mock.conf * 100).toInt()}% conf.',
            style: GoogleFonts.dmMono(fontSize: 9, color: Colors.white54)),
        ]),
      ),
    ),
  ]);
...
class _StatusCard extends StatelessWidget {
  ...
  const _StatusCard({
    required this.scanStates, required this.triageState,
    required this.uploaded, required this.onRunTriage,
    required this.scenario,
    this.onViewInWorklist, this.onNewIntake,
  });

  @override
  Widget build(BuildContext context) {
    if (triageState == _TriageState.complete) return _TriageCompleteCard(scenario: scenario, onViewInWorklist: onViewInWorklist, onNewIntake: onNewIntake);

    final isReady   = uploaded == 4;
    final isRunning = triageState == _TriageState.running;
    final remaining = 4 - uploaded;

    return TCard(padding: const EdgeInsets.all(20), child: Column(children: [
      // Status ring
      AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isReady ? TulipColors.green400 : TulipColors.amber400, width: 3)),
      ...
      const SizedBox(height: 12),
      Text(
        isReady ? 'Ready to analyse'
                : '$remaining view${remaining > 1 ? 's' : ''} remaining',
        style: GoogleFonts.dmSans(
            fontSize: 14, fontWeight: FontWeight.w600, color: TulipColors.text),
      ),
        ...
        return Padding(
          ...
          child: Row(children: [
            AnimatedContainer(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                color: s == _ScanState.done    ? TulipColors.green50
                     : s == _ScanState.loading ? TulipColors.p50
                     : TulipColors.gray50,
                border: Border.all(
                  color: s == _ScanState.done    ? TulipColors.green400
                       : s == _ScanState.loading ? TulipColors.p400
                       : TulipColors.gray100,
                  width: 1.5,
                ),
              ),
              child: s == _ScanState.done
                ? const Icon(Icons.check, size: 10, color: TulipColors.green800)
                : s == _ScanState.loading
                  ? const Padding(
                      padding: EdgeInsets.all(3),
                      child: CircularProgressIndicator(strokeWidth: 1.5, color: TulipColors.p400),
                    )
                  : null,
            ),
            ...
            Text(v, style: GoogleFonts.dmSans(
                fontSize: 12,
                fontWeight: s == _ScanState.done ? FontWeight.w500 : FontWeight.w400,
                color: s == _ScanState.done ? TulipColors.text : TulipColors.textT)),
            const Spacer(),
            Text(
              s == _ScanState.done    ? 'Ready'
            : s == _ScanState.loading ? 'Uploading…'
            : 'Pending',
        ...
        child: isRunning
          ? Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                const SizedBox(
                  width: 14, height: 14,
                  child: CircularProgressIndicator(strokeWidth: 2, color: TulipColors.p600),
                ),
                const SizedBox(width: 8),
                Text('Running AI triage…', style: GoogleFonts.dmSans(
                    fontSize: 13, color: TulipColors.p600, fontWeight: FontWeight.w500)),
              ]),
            )
          : TBtn('Run triage →', primary: isReady, onTap: isReady ? onRunTriage : null),
      ),
    ]));
  }
}
...
class _TriageCompleteCard extends StatelessWidget {
  ...
  final VoidCallback? onViewInWorklist;
  final VoidCallback? onNewIntake;
  const _TriageCompleteCard({required this.scenario, this.onViewInWorklist, this.onNewIntake});
  ...
  Widget build(BuildContext context) {
    final findings = scenario.views.entries
        .where((e) => e.value.label != 'No finding').toList();
    final clearCount = scenario.views.entries
        .where((e) => e.value.label == 'No finding').length;
    ...
    return TCard(padding: const EdgeInsets.all(20), child: Column(children: [
      ...
      const SizedBox(height: 12),
      Text(scenario.resultTitle, style: GoogleFonts.dmSans(
          fontSize: 14, fontWeight: FontWeight.w700, color: _text)),
      const SizedBox(height: 4),
      Text(scenario.resultSub, style: GoogleFonts.dmSans(
      ...
      Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: _accent.withValues(alpha: 0.35), width: 0.5)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...findings.map((e) => Padding(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: Text('${e.key} — ${e.value.label}',
          ...
          if (clearCount > 0)
            Padding(
              padding: EdgeInsets.only(left: findings.isEmpty ? 0 : 15, top: findings.isEmpty ? 0 : 2),
              child: Text(
                clearCount == 4
      ...
      SizedBox(width: double.infinity,
        child: TBtn('View in worklist →', primary: true, onTap: onViewInWorklist)),
      const SizedBox(height: 8),
      SizedBox(width: double.infinity,
...
class _TriageResultBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 8),
      Expanded(child: Text(scenario.bannerText,
        style: GoogleFonts.dmSans(fontSize: 12, color: _text))),
      const SizedBox(width: 8),
      Container(
        padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
        decoration: BoxDecoration(color: _fg, borderRadius: BorderRadius.circular(12)),
        child: Text(scenario.bannerBadge, style: GoogleFonts.dmMono(
...
class _StepNode extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    Color bg; Color fg; Color labelColor;
    switch (state) {
      case _StepState.done:
        bg = TulipColors.p400; fg = Colors.white; labelColor = TulipColors.p400; break;
      case _StepState.active:
        bg = Colors.white; fg = TulipColors.p600; labelColor = TulipColors.p600; break;
      case _StepState.future:
        bg = TulipColors.gray50; fg = TulipColors.textT; labelColor = TulipColors.textT; break;
    }
      ...
      Container(
        ...
        decoration: BoxDecoration(
          ...
          border: state == _StepState.active
            ? Border.all(color: TulipColors.p400, width: 2) : null),
        child: Center(child: Text(display, style: GoogleFonts.dmSans(
...
class _FormField extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text(label.toUpperCase(), style: GoogleFonts.dmSans(
        ...
        color: TulipColors.textS, letterSpacing: 0.6)),
    const SizedBox(height: 5),
    Container(
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9),
      decoration: BoxDecoration(
        color: TulipColors.bg, borderRadius: BorderRadius.circular(8),
        border: Border.all(color: TulipColors.borderMid, width: 0.5)),
      child: Text(value, style: GoogleFonts.dmSans(fontSize: 13, color: TulipColors.text)),
    ),
  ]);
}
    ...
    children: [
      ...
      Container(
        ...
        child: DropdownButton<String>(
          ...
          icon: const Icon(Icons.keyboard_arrow_down, size: 16, color: TulipColors.textS),
          style: GoogleFonts.dmSans(fontSize: 13, color: TulipColors.text),
          dropdownColor: TulipColors.surface,
          items: items.map((s) => DropdownMenuItem(
            value: s,
            child: Text(s, style: GoogleFonts.dmSans(fontSize: 13, color: TulipColors.text)),
          )).toList(),
